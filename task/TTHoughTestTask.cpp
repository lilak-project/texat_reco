...
void TTHoughTestTask::Exec(Option_t *option)
{
    ...
    while(1)
    {
        ...
        else
        {
            for(Int_t iChannel=0; iChannel<mmMult; iChannel++)
            {
                ...
                if(mmpx[mmAsad][mmAget][dchan]==whereisx && !(chan==11 || chan==22 || chan==45 || chan==56))
                {
                    for(Int_t buck=0; buck<MaxBuck; buck++) HWaveFormbyPixel[num_chain] -> Fill(buck,mmWaveformY[buck]);
                    HWaveFormbyPixel[num_chain] -> SetTitle(Form("H%d_%d_%d_%d/%d/%d",mmCobo,mmAsad,mmAget,dchan,mmpx[mmAsad][mmAget][dchan],evt));
                    if(num_chain==0) loc_chain = mmpx[mmAsad][mmAget][dchan];
                    num_chain++;
                    continue;
                }
            }
    ...
    while(1)
    {
        ...
        else
        {
            for(Int_t iChannel=0; iChannel<mmMult; iChannel++)
            {
                ...
                {
                    for(Int_t buck=0; buck<MaxBuck; buck++) HWaveFormbyPixel[num_chain] -> Fill(buck,mmWaveformY[buck]);
                    loc_chain_all[num_chain] = mmpx[mmAsad][mmAget][dchan];
                    num_chain++;
                    continue;
                }
            }
    ...
    while(1)
    {
        ...
        else
        {
            for(Int_t iChannel=0; iChannel<mmMult; iChannel++)
            {
                ...
                if(mmpx[mmAsad][mmAget][dchan]==-1 && mmpy[mmAsad][mmAget][dchan]==whereisz
                        && mmAsad==whichtype[1] && !(chan==11 || chan==22 || chan==45 || chan==56))
                {
                    ...
                    loc_strip_all[num_strip] = mmpy[mmAsad][mmAget][dchan];
                    num_strip++;
                    continue;
                }
            }
    ...
    */
    ...
    TString fileName = Form("hist_data/%s_%04d_%d.root", fRun->GetRunName().Data(), fRun->GetRunID(), evt);
